/**
 * Google ledger receiver v2 — reference source only.
 *
 * This file is intentionally NOT a deployed Apps Script. Deployment, Script
 * Properties, source registration, and any quarantine action require separate
 * explicit approval because the receiver is shared by all production strategies.
 *
 * Required Script Property:
 *   GOOGLE_LEDGER_V2_SOURCES = JSON object keyed by source_id:
 *   {"momentum-wsl-prod":{"hmac_secret":"...","project_id":"mexc-4h-momentum","sheet_name":"mexc-4h-momentum-trailing-stop"}}
 *
 * No source secret belongs in this file, a repository, a command line, or logs.
 */
...
function handleV2(data) {
  if (!data || data.schema_version !== V2_SCHEMA) return {ok: false, error: 'unsupported_schema'};
...
function quarantineOnlyWithExplicitDeploymentFlag(data) {
  // Deliberately fail closed until a separately reviewed fixed quarantine helper
  // and explicit user-confirmation gate are deployed. No row is deleted or moved.
